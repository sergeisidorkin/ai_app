(function () {
  ...
  const CONTACTS_FILTER_TABLES = ['prs-select', 'ctz-select', 'adr-select', 'psn-select', 'tel-select', 'eml-select'];
  ...
  function paneOf(node) {
    return node && node.closest ? node.closest('#contacts-persons-pane, #contacts-citizenships-pane, #contacts-addresses-pane, #contacts-positions-pane, #contacts-phones-pane, #contacts-emails-pane') : null;
  }
  ...
  document.addEventListener('click', function (event) {
    ...
    var panel = button.closest('#prs-actions, #ctz-actions, #adr-actions, #psn-actions, #tel-actions, #eml-actions');
    if (!panel) return;
  ...
  document.body.addEventListener('htmx:configRequest', function (event) {
    ...
    if (!target.matches('#contacts-persons-table-wrap, #contacts-citizenships-table-wrap, #contacts-addresses-table-wrap, #contacts-positions-table-wrap, #contacts-phones-table-wrap, #contacts-emails-table-wrap')) {
      return;
    }
    var values = getSelectedContactsPersonFilterValues();
  ...
  document.body.addEventListener('htmx:configRequest', function (event) {
    ...
    if (!/\/contacts\/(ctz|adr|psn|tel|eml)\/create\/$/.test(path)) return;
    var values = getSelectedContactsPersonFilterValues();
    if (!values.length || values.includes(CONTACTS_PERSON_FILTER_ALL)) {
      if (detail.parameters) delete detail.parameters.prs_ids;
      return;
