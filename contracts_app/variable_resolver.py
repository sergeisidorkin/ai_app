...
def _ep_phone(ep: ExpertProfile, _p: Performer) -> str:
    if not ep:
        return ""
    return getattr(ep.employee, "primary_phone_display", "") or ""


def _ep_extra_email(ep: ExpertProfile, _p: Performer) -> str:
    if not ep:
        return ""
    return ep.resolved_extra_email()


def _ep_extra_phone(ep: ExpertProfile, _p: Performer) -> str:
    if not ep:
        return ""
    return ep.resolved_extra_phone()
...
def _contract_str_field(field_name: str):
    """Return a resolver that reads a CharField / TextField from ExpertContractDetails."""
    def _resolver(ep: ExpertProfile, _p: Performer) -> str:
        details = _contract_details(ep)
        if not details:
            return ""
        return str(getattr(details, field_name, "") or "")
    return _resolver
...
def _contract_date_field(field_name: str, fmt: str = "dd.mm.YYYY"):
    """Return a resolver that formats a DateField from ExpertContractDetails."""
    def _resolver(ep: ExpertProfile, _p: Performer) -> str:
        details = _contract_details(ep)
        if not details:
            return ""
        val = getattr(details, field_name, None)
        if val is None:
            return ""
        if fmt == "j E Y г.":
            from core.dates import format_date_ru
...
def _contract_int_field(field_name: str, suffix: str = ""):
    """Return a resolver that reads an integer field from ExpertContractDetails."""
    def _resolver(ep: ExpertProfile, _p: Performer) -> str:
        details = _contract_details(ep)
        if not details:
            return ""
...
def _gender_display(ep: ExpertProfile, _p: Performer) -> str:
    details = _contract_details(ep)
    if not details or not details.gender:
        return ""
    return details.get_gender_display()
...
FIELD_MAP: dict[tuple[str, str, str], callable] = {
    ...
    ("experts", "experts_base", "extra_email"): _ep_extra_email,
    ("experts", "experts_base", "phone"): _ep_phone,
    ("experts", "experts_base", "extra_phone"): _ep_extra_phone,
    ...
    ("experts", "contract_details", "passport_issued_by"): _contract_str_field("passport_issued_by"),
    ("experts", "contract_details", "passport_issue_date"): _contract_date_field("passport_issue_date", "j E Y г."),
    ("experts", "contract_details", "passport_expiry"): _contract_date_field("passport_expiry_date", "dd.mm.YYYY"),
    ("experts", "contract_details", "passport_division_code"): _contract_str_field("passport_division_code"),
    ("experts", "contract_details", "registration_address"): _contract_str_field("registration_address"),
    ("experts", "contract_details", "bank_name"): _contract_str_field("bank_name"),
    ("experts", "contract_details", "swift"): _contract_str_field("bank_swift"),
    ("experts", "contract_details", "bank_inn"): _contract_str_field("bank_inn"),
    ("experts", "contract_details", "bik"): _contract_str_field("bank_bik"),
    ("experts", "contract_details", "settlement_account"): _contract_str_field("settlement_account"),
    ("experts", "contract_details", "correspondent_account"): _contract_str_field("corr_account"),
    ("experts", "contract_details", "bank_address"): _contract_str_field("bank_address"),
    ("experts", "contract_details", "corr_bank_name"): _contract_str_field("corr_bank_name"),
    ("experts", "contract_details", "corr_bank_address"): _contract_str_field("corr_bank_address"),
    ("experts", "contract_details", "corr_bank_bik"): _contract_str_field("corr_bank_bik"),
...
def _calc_contract_price(ep, all_performers):
    ...
    tax_rate = 0
    details = _contract_details(ep)
    if details and details.tax_rate is not None:
        tax_rate = int(details.tax_rate)
...
def _computed_named(ep, _p, _all_performers) -> str:
    ...
    if not details or not details.gender:
        return ""
    return "именуемый" if details.gender == "male" else "именуемая"
