...
def _list_children(user, path):
    try:
        return list_folder_resources(user, path, limit=2000) or []
    except CloudStorageNotReadyError:
        raise


def list_cloud_tree(user, root_path):
    ...
    while queue:
        current = queue.pop(0)
        children = _list_children(user, current)
...
def materialize_inbox_tree(inbox_root: Path, user, source_folder, section_label):
